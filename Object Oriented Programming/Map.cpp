...
void Map::getPollEvent(){                   //Responsible for Inputs and other Events.
    while (this->window->pollEvent(ev)){
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Escape))               //Window close using ESC key.
            window->close();
        if (ev.type == sf::Event::KeyReleased && p->getIsAlive() == true)   //FrameTime reset and Jump key input.
            m->setDeltaTime();
                                            // Checks if Up arrow is pressed to Jump.
        if (ev.type == sf::Event::KeyPressed && ev.type != sf::Event::KeyReleased && p->getIsAlive() == true)
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up))
                p->isJumping = true;                                        //Sets bool in player class as true.
...
void Map::update() {
    if (p->getIsAlive() == true){                       //If this condition is false, Loop doesnt run. Game gets paused.
        ...
        timeDelta = 0.02f;                  //Assigns value to timeDelta to be sent to the playerController.
